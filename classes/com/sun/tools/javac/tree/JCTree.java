 ...
 * specific tree nodes as subclasses nested inside.
 *
 * <p>
 ...
 *
 * <p>
 * Except for the methods defined by com.sun.source, the only method defined in
 * subclasses is `visit' which applies a given visitor to the tree. The actual
 * tree processing is done by visitor classes in other packages. The abstract
 * class Visitor, as well as an Factory interface for trees, are defined as
 * inner classes in Tree.
 *
 * <p>
 * To avoid ambiguities with the Tree API in com.sun.source all sub classes
 * should, by convention, start with JC (javac).
 *
 * <p>
...
public abstract class JCTree implements Tree, Cloneable, DiagnosticPosition {
	 ...
	 * @param defs             All definitions in this file (ClassDef, Import, and
	 *                         Skip)
	 ...
	 * @param lineMap          Line starting positions, defined only if option -g is
	 *                         set.
	 * @param docComments      A hashtable that stores all documentation comments
	 *                         indexed by the tree nodes they refer to. defined only
	 *                         if option -s is set.
	...
	public static class JCCompilationUnit extends JCTree implements CompilationUnitTree {
		...
		protected JCCompilationUnit(List<JCAnnotation> packageAnnotations, JCExpression pid, List<JCTree> defs,
				JavaFileObject sourcefile, PackageSymbol packge, ImportScope namedImportScope,
	...
	public static class JCClassDecl extends JCStatement implements ClassTree {
		...
		protected JCClassDecl(JCModifiers mods, Name name, List<JCTypeParameter> typarams, JCExpression extending,
				List<JCExpression> implementing, List<JCTree> defs, ClassSymbol sym) {
			this.mods = mods;
			this.name = name;
	...
	public static class JCMethodDecl extends JCTree implements MethodTree {
		...
		protected JCMethodDecl(JCModifiers mods, Name name, JCExpression restype, List<JCTypeParameter> typarams,
	...
	public static class JCVariableDecl extends JCStatement implements VariableTree {
		...
		protected JCVariableDecl(JCModifiers mods, Name name, JCExpression vartype, JCExpression init, VarSymbol sym) {
			this.mods = mods;
			this.name = name;
	...
	public static class JCForLoop extends JCStatement implements ForLoopTree {
		...
		protected JCForLoop(List<JCStatement> init, JCExpression cond, List<JCExpressionStatement> update,
				JCStatement body) {
			this.init = init;
			this.cond = cond;
			this.step = update;
	...
	public static class JCNewClass extends JCExpression implements NewClassTree {
		...
		protected JCNewClass(JCExpression encl, List<JCExpression> typeargs, JCExpression clazz,
	...
	public static class JCBinary extends JCExpression implements BinaryTree {
		...
		protected JCBinary(int opcode, JCExpression lhs, JCExpression rhs, Symbol operator) {
			this.opcode = opcode;
			this.lhs = lhs;
			this.rhs = rhs;
			this.operator = operator;
	...
	public static class JCErroneous extends JCExpression implements com.sun.source.tree.ErroneousTree {
	...
	public interface Factory {
		JCCompilationUnit TopLevel(List<JCAnnotation> packageAnnotations, JCExpression pid, List<JCTree> defs);

		JCImport Import(JCTree qualid, boolean staticImport);

		JCClassDecl ClassDef(JCModifiers mods, Name name, List<JCTypeParameter> typarams, JCExpression extending,
				List<JCExpression> implementing, List<JCTree> defs);

		JCMethodDecl MethodDef(JCModifiers mods, Name name, JCExpression restype, List<JCTypeParameter> typarams,
				List<JCVariableDecl> params, List<JCExpression> thrown, JCBlock body, JCExpression defaultValue);
		...
		JCForLoop ForLoop(List<JCStatement> init, JCExpression cond, List<JCExpressionStatement> step,
				JCStatement body);
		...
		JCNewClass NewClass(JCExpression encl, List<JCExpression> typeargs, JCExpression clazz, List<JCExpression> args,
				JCClassDecl def);
